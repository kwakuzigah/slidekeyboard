...
public class CandidateView extends View {
    ...
    private SlideKeyboard                 mService;
    private List<Suggestion>              mSuggestions                = EMPTY_LIST;
    ...
    private int                           mColorUser;
    private Paint                         mPaint;
    private int                           mDescent;
    private boolean                       mScrolled;
    private boolean                       mIsDisplayingContextualMenu = false;
	...
	mPreviewText = (TextView) inflate.inflate(R.layout.candidate_preview,
	        null);
	mPreviewText.setTypeface(SlideKeyboard.textTypeFace);
	...
	mColorUser = context.getResources().getColor(R.color.candidate_user);
	...
	mGestureDetector = new GestureDetector(
		    ...
		    public void onLongPress(MotionEvent me) {
			    ...
			    if (mSelectedString != null) {
			        mService.showCandidatesContextualMenu(mSelectedString);
			        mIsDisplayingContextualMenu = true;
			    }
		        }
     ...
     * @param listener
     */
    public void setService(SlideKeyboard listener) {
	mService = listener;
    }
	...
	for (int i = 0; i < count; i++) {
	    ...
	    if (suggestion == null)
		continue;
	    CharSequence word = suggestion.word;
	    boolean isUserWord = suggestion.isUserWord;
	    if (word == null)
		continue;
	    paint.setColor(mColorNormal);
	    if (mHaveMinimalSuggestion
		    && ((i == 1 && !typedWordValid) || (i == 0 && typedWordValid))) {
		paint.setTypeface(SlideKeyboard.boldTextTypeFace);
		paint.setColor(mColorRecommended);
	    } else if (isUserWord) {
		paint.setColor(mColorUser);
	    } else if (i != 0) {
		...
		float textWidth = paint.measureText(word, 0, word.length());
		...
		mSelectedString = word;
		mSelectedIndex = i;
	    }

	    if (canvas != null) {
		canvas.drawText(word, 0, word.length(), x + X_GAP, y, paint);
		paint.setColor(mColorOther);
		canvas.translate(x + wordWidth, 0);
		mDivider.draw(canvas);
		canvas.translate(-x - wordWidth, 0);
	    }
	    paint.setTypeface(SlideKeyboard.textTypeFace);
	    x += wordWidth;
	}
	mTotalWidth = x;
	...
	case MotionEvent.ACTION_DOWN:
	    mScrolled = false;
	    mIsDisplayingContextualMenu = false;
	    invalidate();
	    break;
	case MotionEvent.ACTION_MOVE:
		...
		if (mSelectedString != null) {
			...
			TextEntryState.acceptedSuggestion(
			        mSuggestions.get(0).word, mSelectedString);
		    }
	...
	case MotionEvent.ACTION_UP:
	    if (!mIsDisplayingContextualMenu && !mScrolled) {
		if (mSelectedString != null) {
		    if (!mShowingCompletions) {
			TextEntryState.acceptedSuggestion(
			        mSuggestions.get(0).word, mSelectedString);
	...
	mHandler.sendMessageDelayed(
	        mHandler.obtainMessage(MSG_REMOVE_THROUGH_PREVIEW), 200);
    }
	...
	if (mPreviewPopup.isShowing()) {
	    mHandler.sendMessageDelayed(
		    mHandler.obtainMessage(MSG_REMOVE_PREVIEW), 60);
	}
    }
    ...
    // showPreview(0, getContext().getResources().getString(R.string.added_word,
    // word));
    // }
    // }
